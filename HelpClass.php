...
<?php
class FormData {
  // public function prepare_clientMail (){
  //
  // }
  public function prepare_html_personalData($data){
    ...
    <p>    <strong >Alojamento:</strong>
          <cite id="alojamento"><i>'.$data['alojamento'].'</i></cite>
      </p>
  <p>
      <strong >Nome:</strong>
      <cite id="nome">'.$data['name'].'</cite>
</p>
<p>
      <strong>Email:</strong>
      <cite id="email">'.$data['email'].'</cite>
      </p>
      <p>
      <strong>Telefone:</strong>
      <cite id="phone">'.$data['phone'].'</cite>
      </p>
      <p>
      <strong>Ilha:</strong>
      <cite id="ilha">'.$data['ilha'].'</cite>
      </p>
      <p>
      <strong>Concelho:</strong>
      <cite id="concelho">'.$data['concelho'].'</cite>
      </p>
      <p>
      <strong>Cartão de Cidadão:</strong>
      <cite id="cc">'.$data['cc'].'</cite>
      </p>
      <p>
      <strong>Nº Identidade Fiscal:</strong>
      <cite id="nif">'.$data['nif'].'</cite>
      </p>
      <strong>Rede Social:</strong>
      <cite id="nif">'.$data['social'].'</cite>
      </p>

    ';

    /*-----------------*/
      if($data['ref']=='1' &&  isset($data['universidade']) && isset($data['curso']))
      {
        $html.='<p><strong>Curso:</strong>
        <cite id="curso">'.$data['curso'].'</cite>
</p><p>
        <strong>Universidade:</strong>
        <cite id="universidade">'.$data['universidade'].'</cite></p>
        ';
      }
      /*-----------------*/
      $html.='<hr />';
  ...
  public function prepare_JSON_personalData($data){
    $inp = file_get_contents('inscritos.json');
    $tempArray = json_decode($inp);
  if(empty($tempArray) ){
    $json[] = $data;
    $fp = fopen('inscritos.json', 'a');
    ...
    file_put_contents('inscritos.json', $jsonData);
  }
  }
}
